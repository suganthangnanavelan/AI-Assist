...
const CorpusManager = () => {
  ...
  const fetchIntents = async () => {
    const result = await axios.get('http://localhost:3001/intents');
  ...
  const addOrUpdateIntent = async () => {
    ...
    if (editingId) {
      await axios.put(`http://localhost:3001/intents/${editingId}`, newIntent);
    } else {
      await axios.post('http://localhost:3001/intents', newIntent);
    }
  ...
  const deleteIntent = async (id) => {
    await axios.delete(`http://localhost:3001/intents/${id}`);
    fetchIntents();
  };
